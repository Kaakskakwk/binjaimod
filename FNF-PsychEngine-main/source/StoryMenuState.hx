...
class StoryMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		scoreText = new FlxText(10, 500, 0, "SCORE: 49324858", 36);
		...
		var BG = new FlxSprite(0, -75);
		BG.frames = Paths.getSparrowAtlas('binjaiBG');
		...
		var left:FlxSprite = new FlxSprite(0, -500).loadGraphic(Paths.image('left'));
		// left.scrollFactor.set(0, yScroll);
		left.setGraphicSize(Std.int(left.width * 1.175));
		left.updateHitbox();
		left.screenCenter();
		left.antialiasing = ClientPrefs.globalAntialiasing;
		add(left);

		var right:FlxSprite = new FlxSprite(-10, -500).loadGraphic(Paths.image('right'));
		// left.scrollFactor.set(0, yScroll);
		right.setGraphicSize(Std.int(right.width * 1.175));
		right.updateHitbox();
		right.screenCenter();
		...
		var tracksSprite:FlxSprite = new FlxSprite(550, 500).loadGraphic(Paths.image('Menu_Tracks'));
		tracksSprite.antialiasing = ClientPrefs.globalAntialiasing;
		// add(tracksSprite);

		txtTracklist = new FlxText(FlxG.width * 0.05, tracksSprite.y + 60, 0, "", 42);
