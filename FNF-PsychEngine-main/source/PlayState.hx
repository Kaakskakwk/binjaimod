...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'binjai': //week binjai
			var bgb:BGSprite = new BGSprite('gameplaybg/mountain', -600, -200, 0.9, 0.9);
			add(bgb);

			var bg:BGSprite = new BGSprite('gameplaybg/tree', -600, -200, 0.9, 0.9);
			add(bg);
			

			case 'firebinjai': //week blackbinjai
			var fire:BGSprite = new BGSprite('gameplaybg/city', -600, -100, 0.9, 0.9);
			add(fire);

			var firee:BGSprite = new BGSprite('gameplaybg/ground', -600, -100, 0.9, 0.9);
			add(firee);

			case 'maghrib':
			var bruh:BGSprite = new BGSprite('gameplaybg/mountainmaghrib', -600, -200, 0.9, 0.9);
			add(bruh);

			var shitman:BGSprite = new BGSprite('gameplaybg/treemaghrib', -600, -200, 0.9, 0.9);
			add(shitman);
		...
		psychEngineWatermark = new FlxText(5, healthBarBG.y + 50, "Psych Engine 0.5.1", 16);
		...
		if(ClientPrefs.downScroll) psychEngineWatermark.y = FlxG.height * 0.9 + 70;

		tips = new FlxText(5, healthBarBG.y + 50, "Press F1 to enable hitsound / Press F2 to unable hit sound", 16);
		tips.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		tips.scrollFactor.set();
		// add(tips);
		if(ClientPrefs.downScroll) tips.y = FlxG.height * 0.9 + 70;
		...
		if (isStoryMode && !seenCutscene)
		{
			switch (daSong)
			{
				...
				case 'senpai' | 'roses' | 'thorns':
					...
					case 'the-greeting':
						startVideo('thegreeting');

					case 'binjai-town':
						startVideo('binjaitown');

					case 'polargeist':
						startVideo('polargeist');

					case 'archipelago':
						startVideo('archipelago');

					case 'potassium-punch':
						startVideo('potassium');
