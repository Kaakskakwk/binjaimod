...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		if (titleJSON.backgroundSprite != null && titleJSON.backgroundSprite.length > 0 && titleJSON.backgroundSprite != "none"){
			...
			bg.loadGraphic(Paths.image('binjaititle'));
		}
		...
		logoBl.animation.addByPrefix('bump', 'Start Screen BG art', 24, false);
		logoBl.animation.play('bump');
		logoBl.updateHitbox();
		// logoBl.screenCenter();
		// logoBl.color = FlxColor.BLACK;

		bg3 = new FlxSprite().loadGraphic(Paths.image('binjaititle'));
		// bg.antialiasing = ClientPrefs.globalAntialiasing;
		// bg.setGraphicSize(Std.int(bg.width * 0.6));
		// bg.updateHitbox();
		add(bg3);

		logoB = new FlxSprite(600, -130);
		logoB.frames = Paths.getSparrowAtlas('logoBumpin');
		logoB.antialiasing = ClientPrefs.globalAntialiasing;
		logoB.animation.addByPrefix('bump', 'Start Screen BG art', 24);
		logoB.animation.play('bump');
		logoB.updateHitbox();
		logoB.screenCenter(X);
		add(logoB);

		pressenter = new FlxSprite(0, 0).loadGraphic(Paths.image('pressenter'));
		pressenter.screenCenter();
		// add(pressenter);

		bg2 = new FlxSprite(-300, 300);
		bg2.frames = Paths.getSparrowAtlas('Binjai_Bop');
		bg2.antialiasing = ClientPrefs.globalAntialiasing;
		bg2.animation.addByPrefix('Binjai Man Bop', 'Binjai Man Bop', 24);
		bg2.animation.play('Binjai Man Bop');
		bg2.updateHitbox();
		bg2.screenCenter(Y);
		add(bg2); //600 will move to the right

		
		bf = new FlxSprite(800, 0);
		bf.frames = Paths.getSparrowAtlas('characters/BOYFRIEND', 'shared');
		bf.antialiasing = ClientPrefs.globalAntialiasing;
		bf.animation.addByPrefix('bf pre attack', 'bf pre attack', 24);
		bf.animation.play('bf pre attack');
		bf.updateHitbox();
		bf.screenCenter(Y);
		add(bf); 

		bf2 = new FlxSprite(800, 0);
		bf2.frames = Paths.getSparrowAtlas('characters/BOYFRIEND', 'shared');
		bf2.antialiasing = ClientPrefs.globalAntialiasing;
		bf2.animation.addByPrefix('BF HEY!!', 'BF HEY!!', 24);
		bf2.animation.play('BF HEY!!');
		bf2.updateHitbox();
		bf2.screenCenter(Y);
		...
		var logo:FlxSprite = new FlxSprite(600, -130).loadGraphic(Paths.image('logo'));
		logo.screenCenter();
		...
		ngSpr = new FlxSprite(0, FlxG.height * 0.52).loadGraphic(Paths.image('lmaobangetngab'));
		add(ngSpr);
		ngSpr.visible = false;
	...
	override function update(elapsed:Float)
	{
		...
		if (!transitioning && skippedIntro)
		{
			if(pressedEnter)
			{
				...
				FlxTween.tween(bg3, {angle: 45}, 0.9, {ease: FlxEase.expoIn});
				FlxTween.tween(bg3, {x: 1000}, 1, {ease: FlxEase.expoIn});
				FlxTween.tween(logoB, {y: -600}, 0.6, {ease: FlxEase.expoIn});
				FlxTween.tween(bg2, {x: 1000}, 0.6, {ease: FlxEase.expoIn});
				FlxTween.tween(bf2, {x:-1000}, 0.6, {ease: FlxEase.expoIn});
				add(bf2);
				remove(bf);
	...
	override function beatHit()
	{
		...
		if(!closedState) {
			...
			{
				case 1:
					createCoolText(['Binjai Mod By'], 15);	
				// credTextShit.visible = true;
				case 3:
					addMoreText('Binjai Dev', 15);
				// credTextShit.text += '\npresent...';
				...
				// credTextShit.screenCenter();
				case 5:
					createCoolText(['Fuck you I', 'eat vegetables'], -40);
				case 7:
					addMoreText('meat is killing animal', -40);
					ngSpr.visible = true;
				// credTextShit.text += '\nNewgrounds';
				case 8:
					deleteCoolText();
				...
				// credTextShit.text += '\nNight';
				case 15:
					addMoreText('Binjai'); // credTextShit.text += '\nFunkin';
